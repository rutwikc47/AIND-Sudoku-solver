...
diag1=list(rows[i]+cols[i] for i in range(len(rows)))
...
def naked_twins(values):
    ...
    if sameval:
        count = 0
        for ele in peerlist:
            digit = values[diglist[count]]
            for e in ele:
                if len(values[e]) > 2:
                    values[e] = values[e].replace(digit[0], '')
                    values[e] = values[e].replace(digit[1], '')
            count += 1
